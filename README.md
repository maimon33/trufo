...
Trufo ships a Progressive Web App at `/app` — same Lambda, no extra infrastructure. Works on iOS and Android.

- **Install**: open `yourdomain.com/app` in mobile Safari or Chrome → "Add to Home Screen"
- **Auth**: passwordless via magic link email or 6-digit OTP (no password, no Google account)
- **Share**: native share sheet — tap Share, pick WhatsApp / iMessage / copy link
- **Offline shell**: loads instantly even without signal, API calls always go live

Source lives in `pwa/` and is built + bundled into the Lambda at deploy time.
...
│   ├── pwa_static/           # PWA build output (generated by CI, gitignored)
│   └── requirements.txt      # Python dependencies
├── pwa/                      # Mobile PWA source (React + Vite)
